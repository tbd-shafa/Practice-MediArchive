...
const ImageZoomModal: FC<ImageZoomModalProps> = ({
  imageUrl,
  onClose,
  prescriptionId,
  patientId,
  ...
  const [showDeleteModalNotePrescription, setShowDeleteModalNotePrescription] =
    useState(false);
...
const handleImageLoad = () => {
  ...
  const handleDeleteNotePrescription = async () => {
    try {
      const response = await PrescriptionService.deletePrescriptionFromNote(
        noteId,
        prescriptionId
      );
      if (response.status === "success") {
        setShowDeleteModalNotePrescription(false);
        toast.success("Prescription Deleted From Note successfully");
      ...
      toast.error("Failed to delete prescription from note");
    }
  };

  const handleImageDelete = async () => {
    if (images.length === 1) {
      toast.error("Sorry!..Prescription must have at least one image.");
  ...
  return (
    <>
          ...
          <div className="image-controls">
            {!fromNoteView && ( // Conditionally render based on the new prop
              <>
                ...
                <button
                  ...
                  onClick={() =>
                    router.push(
                      `/view-patient/${patientId}?tab=prescription&view=edit&prescriptionId=${prescriptionId}`
                    )
                  }
                >
                  <Image
            ...
            {fromNoteView && (
              <button
                ...
                className="remove-btn"
                title="Delete"
                onClick={() => setShowDeleteModalNotePrescription(true)}
              >
                <i className="fas fa-trash"></i>
                <Image
                  src="/images/delete-icon-white.svg"
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body pt-5 pb-3">
                <h4>
                  Are you sure you <br /> want to Delete This Prescription?
                </h4>
                <h4>
      ...
      {showDeleteModalNotePrescription && (
        <div
          className="modal fade confirm-modal patient_remove_cm show"
          style={{ display: "block" }}
        >
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body pt-5 pb-3">
                <h4>
                  Are you sure you <br /> want to Delete This Prescription From
                  Note?
                </h4>
              </div>
              <div className="modal-footer px-4">
                <button
                  type="button"
                  className="btn btn-secondary"
                  onClick={() => setShowDeleteModalNotePrescription(false)}
                >
                  NO KEEP THAT
                </button>
                <button
                  type="button"
                  className="btn btn-primary"
                  onClick={handleDeleteNotePrescription}
                  data-bs-dismiss="modal"
                >
                  YES DELETE
                </button>
