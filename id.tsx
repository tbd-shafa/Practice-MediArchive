...
const PatientView: NextPage = () => {
  ...
  const { id: patientName, view, tab } = router.query;
  const [patient, setPatient] = useState<Patient | null>(null);
  const [activeTab, setActiveTab] = useState("blood-pressure");

  // Get ID from localStorage
  const id = typeof window !== 'undefined' ? localStorage.getItem('current_patient_id') : null;
  ...
  const handleTabClick = (tabName: string) => {
    ...
    router.push(
      {
        pathname: `/view-patient/${patientName}`,
        query: { tab: tabName },
      },
      undefined,
      { shallow: true }
  ...
  const clearLabReportEditData = () => {
    ...
    localStorage.removeItem("selectedDateAddLabReport");
    localStorage.removeItem("labReportImages");
  ...
  useEffect(() => {
    ...
    const isLabReportAddRoute =
      view === "add" &&
      activeTab === "lab-report" &&
      !router.query.addTags &&
      !router.query.imageAttachment;

    const isLabReportAddTagsRoute =
      view === "add" &&
      activeTab === "lab-report" &&
      router.query.addTags === "true";

    const isLabReportAddAttachmentRoute =
      view === "add" &&
      activeTab === "lab-report" &&
      router.query.imageAttachment === "true";

    const isInLabReportFlow =
      isLabReportEditRoute ||
      isLabReportEditTagsRoute ||
      isLabReportEditAttachmentRoute ||
      isLabReportAddRoute ||
      isLabReportAddTagsRoute ||
      isLabReportAddAttachmentRoute;

    if (!isInLabReportFlow) {
  ...
  const clearPrescriptionEditData = () => {
    ...
    localStorage.removeItem("selectedDateEditPrescription");
    localStorage.removeItem("removedPrescriptionApiImages");
    localStorage.removeItem("removedPrescriptionSymptoms");
    localStorage.removeItem("selectedPrescriptionAddDcotors"); // Clear add doctors
    localStorage.removeItem("selectedDateAddPrescription"); // Clear add prescription date
  };
  ...
  const renderContent = () => {
    ...
    if (view === "edit" && activeTab === "lab-report") {
      ...
      return (
        <LabReportEdit patientId={id} labReportId={labReportId as string} />
      );
    }
    ...
    if (view === "reminder" && activeTab === "medicine") {
      const { medicineid } = router.query;
      return (
        <MedicineReminder patientId={id} medicineId={medicineid as string} />
      );
    }
    // Handle main views
    switch (activeTab) {
            ...
            <div className="max-width-761px">
              <div className="patient-view-card">
                <div className="patient-view-sidebar">
                                ...
                                <>
                                  ...
                                  <img
                                    ...
                                    onError={() => {
                                      ...
                                      placeholderImage.alt = "User";
                                      placeholderImage.width = 80;
                                      placeholderImage.height = 80;
                                      placeholderImage.style.objectFit =
                                        "cover";
                                      placeholderImage.style.borderRadius =
                                        "8px";
                                      placeholderImage.style.position =
                                        "relative";
                  ...
                  <div className="card border-radius-15" style={{ height: "58.3vh" }}>
