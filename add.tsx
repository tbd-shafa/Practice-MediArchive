...
const PrescriptionAdd: FC<PrescriptionAddProps> = ({ patientId }) => {
  ...
  useEffect(() => {
    const handleRouteChange = (url: string) => {
      ...
      const addTags = parsedUrl.searchParams.get("addTags");
      const imageAttachment = parsedUrl.searchParams.get("imageAttachment");

      // Check if we're staying in the prescription add flow
      const isInPrescriptionAddFlow = 
        view === "add" && 
        tab === "prescription" &&
        (
          (!addTags && !imageAttachment) || // main add page
          addTags === "true" || // add tags page
          imageAttachment === "true" // add image page
        );

      if (!isInPrescriptionAddFlow) {
        // Clear localStorage only if leaving the prescription add flow
