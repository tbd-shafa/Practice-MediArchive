...
const PrescriptionEdit: React.FC<PrescriptionEditProps> = ({
    ...
    const fetchPrescription = async () => {
      try {
        ...
        if (data.symptoms && Array.isArray(data.symptoms)) {
          ...
          const parsedStoredSymptoms = storedSymptoms ? JSON.parse(storedSymptoms) : [];
          
          // Combine API symptoms with any stored symptoms, removing duplicates
          const combinedSymptoms = [...data.symptoms, ...parsedStoredSymptoms].filter(
            (symptom, index, self) => 
              index === self.findIndex((s) => s.id === symptom.id)
          );
          
          setSelectedSymptoms(combinedSymptoms);
          localStorage.setItem("selectedSymptoms", JSON.stringify(combinedSymptoms));
        }
  ...
  useEffect(() => {
    const handleRouteChange = (url: string) => {
      ...
      const isNotPrescriptionAddPage =
        view !== "edit" || tab !== "prescription";
                ...
                {formData.images.length > 0 ? (
                  <Slider ref={sliderRef} {...sliderSettings}>
                    {formData.images.map((image, index) => (
                      ...
                      >
                        <img
                          src={image.url} // Use the image URL
                        ...
                        <button
                          ...
                          onClick={() => removeImage(index, image.url)} // Pass the image URL here
